...
@php
    ...
    $fieldOrder = [
        ...
        'contact' => ['company_name','address','phone','mobile','email','support_email','sales_email','google_maps_embed'],
    ...
    $labels = [
        ...
        'contact.company_name' => 'Company Name', 'contact.google_maps_embed' => 'Google Maps Embed',
                    ...
                    @foreach($items as $setting)
                        ...
                        <div class="{{ $column }}">
                            @if($setting->type === 'boolean')
                                ...
                                <label class="form-check mt-4" for="{{ $inputId }}"><input class="form-check-input" id="{{ $inputId }}" type="checkbox" name="settings[{{ $key }}]" value="1" @checked(old('settings.'.$group.'.'.$setting->key, (bool)$setting->value))> {{ $label }}</label>
                            @else
                                <label class="form-label" for="{{ $inputId }}">{{ $label }}</label>
                                @if($setting->type === 'image')
                                    @if($setting->value)<div><img src="{{ Storage::url($setting->value) }}" alt="Current {{ $label }}" loading="lazy" class="img-thumbnail mb-2 setting-image-preview"></div>@endif
                                    <input class="form-control @error('image_settings.'.$key) is-invalid @enderror" id="{{ $inputId }}" name="image_settings[{{ $key }}]" type="file" accept=".png,.jpg,.jpeg,.webp,.ico">
                                    @error('image_settings.'.$key)<div class="invalid-feedback">{{ $message }}</div>@enderror
                                @elseif($setting->type === 'text')
                                    <textarea class="form-control @error('settings.'.$key) is-invalid @enderror" id="{{ $inputId }}" name="settings[{{ $key }}]" rows="{{ str_contains($setting->key, 'embed') ? 5 : 3 }}">{{ $value }}</textarea>
                                @elseif($setting->key === 'theme_color')
                                    <input class="form-control form-control-color @error('settings.'.$key) is-invalid @enderror" id="{{ $inputId }}" name="settings[{{ $key }}]" type="color" value="{{ $value ?: '#0d6efd' }}">
                                @elseif(str_contains($setting->key, 'estimated_return'))
                                    <input class="form-control @error('settings.'.$key) is-invalid @enderror" id="{{ $inputId }}" name="settings[{{ $key }}]" type="datetime-local" value="{{ $value ? \Illuminate\Support\Carbon::parse($value)->format('Y-m-d\TH:i') : '' }}">
                                @elseif(str_contains($setting->key, 'encryption'))
                                    <select class="form-select @error('settings.'.$key) is-invalid @enderror" id="{{ $inputId }}" name="settings[{{ $key }}]"><option value="">None</option><option value="tls" @selected($value === 'tls')>TLS</option><option value="ssl" @selected($value === 'ssl')>SSL</option></select>
                                @elseif($setting->key === 'robots_meta')
                                    <select class="form-select @error('settings.'.$key) is-invalid @enderror" id="{{ $inputId }}" name="settings[{{ $key }}]">@foreach(['index,follow','index,nofollow','noindex,follow','noindex,nofollow'] as $option)<option value="{{ $option }}" @selected($value === $option)>{{ $option }}</option>@endforeach</select>
                                @else
                                    <input class="form-control @error('settings.'.$key) is-invalid @enderror" id="{{ $inputId }}" name="settings[{{ $key }}]" type="{{ in_array($setting->type, ['email','url']) ? $setting->type : ($setting->type === 'integer' ? 'number' : ($setting->type === 'secret' ? 'password' : 'text')) }}" value="{{ $value }}" @if($setting->type === 'secret') autocomplete="new-password" placeholder="Leave blank to keep current value" @endif>
                                @endif
