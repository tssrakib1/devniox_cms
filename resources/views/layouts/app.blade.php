<!doctype html>
<html lang="{{ $siteSettings['general.default_language'] ?? app()->getLocale() }}" data-bs-theme="dark">
<head>
    <meta charset="utf-8">
...
<footer class="public-footer"><div class="container"><div class="footer-grid"><div class="footer-brand"><a class="navbar-brand" href="{{ route('home') }}">@if(filled($siteSettings['branding.logo'] ?? null))<img class="brand-logo brand-logo-light" src="{{ Storage::url($siteSettings['branding.logo']) }}" alt="{{ $companyName }}">@if(filled($siteSettings['branding.dark_logo'] ?? null))<img class="brand-logo brand-logo-dark" src="{{ Storage::url($siteSettings['branding.dark_logo']) }}" alt="{{ $companyName }}">@endif @else<span class="brand-mark" aria-hidden="true">{{ Str::upper(Str::substr($siteName,0,1)) }}</span> {{ $siteName }}@endif</a><p>{{ $cmsFooter?->short_description }}</p><div class="footer-socials">@foreach($socialLinks as $link)<a href="{{ $link->url }}" target="_blank" rel="noopener noreferrer" aria-label="{{ ucfirst($link->platform) }}"><i class="bi bi-{{ $link->platform === 'x' ? 'twitter-x' : $link->platform }}" aria-hidden="true"></i></a>@endforeach</div></div><nav class="footer-links" aria-label="Footer navigation"><div><h2>Explore</h2>@foreach($footerNavigation as $item)<a href="{{ $item->url }}" @if($item->open_new_tab) target="_blank" rel="noopener" @endif>{{ $item->label }}</a>@endforeach</div><div><h2>{{ $cmsFooter?->company_heading ?: 'Company' }}</h2><a href="{{ route('about') }}">About</a><a href="{{ route('contact', [], false) }}">Contact</a><a href="{{ route('demo-request') }}">Request demo</a><a href="{{ route('quote-request') }}">Request quote</a></div><div><h2>{{ $cmsFooter?->resources_heading ?: 'Resources' }}</h2><a href="{{ route('blog') }}">Knowledge center</a><a href="{{ route('blog.rss') }}">RSS feed</a><a href="{{ route('sitemap') }}">Sitemap</a></div></nav><div class="footer-contact"><h2>{{ $cmsFooter?->conversation_heading ?: 'Start a conversation' }}</h2><p>{{ $cmsFooter?->conversation_description ?: $cmsFooter?->short_description }}</p>@if(filled($cmsFooter?->contact_email))<a href="mailto:{{ $cmsFooter->contact_email }}">{{ $cmsFooter->contact_email }}</a>@endif @if(filled($cmsFooter?->contact_phone))<a href="tel:{{ preg_replace('/[^0-9+]/', '', $cmsFooter->contact_phone) }}">{{ $cmsFooter->contact_phone }}</a>@endif</div></div><div class="footer-bottom"><small>{{ $cmsFooter?->copyright }}</small><span>{{ $cmsFooter?->bottom_right_text }}</span></div></div></footer>
